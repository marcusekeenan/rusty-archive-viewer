...
interface PenPropertiesDialogProps {
  ...
  onSave: (properties: PenProperties, axisId: string) => void;
}
...
export default function PenPropertiesDialog(props: PenPropertiesDialogProps) {
  ...
  const [selectedAxisId, setSelectedAxisId] = createSignal(props.selectedAxisId || '');
  
  const debouncedSave = debounce((newProps: PenProperties, axisId: string) => {
    props.onSave(newProps, axisId);
  }, 150);

  createEffect(() => {
    if (props.isOpen) {
      setProperties(props.properties);
      setSelectedAxisId(props.selectedAxisId || '');
    }
  });
  ...
  const updateProperty = <K extends keyof PenProperties>(
    ...
    debouncedSave(newProps, selectedAxisId());
  };
    ...
    <div class="fixed inset-0 z-50">
      ...
      <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] bg-white rounded-lg shadow-lg">
        <div class="flex items-center justify-between p-4 border-b">
          <h2 class="text-lg font-semibold">Properties: {props.pv}</h2>
          <button 
            onClick={props.onClose}
        ...
        <div class="p-4 space-y-6">
          {/* Axis Selection */}
          <div class="space-y-2">
            <label class="block text-sm font-medium text-gray-700">Axis Assignment</label>
            <select
              value={selectedAxisId()}
              onChange={(e) => {
                const newAxisId = e.currentTarget.value;
                setSelectedAxisId(newAxisId);
                debouncedSave(properties(), newAxisId);
              }}
          ...
          <div class="border-t pt-4">
            <h3 class="text-sm font-medium text-gray-700 mb-4">Line Properties</h3>
